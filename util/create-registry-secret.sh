...
then 
    printf "%s %s %s %s %s %s\n" "$0" '${CICD_NAMESPACE}' '${CONTAINER_REGISTRY_SERVER}' '${CONTAINER_REGISTRY_ORG}' '${CONTAINER_REGISTRY_USERNAME}' '${CONTAINER_REGISTRY_PASSWORD}' '${CONTAINER_REGISTRY_SECRET_NAME}'
    exit 1
fi
...
CONTAINER_REGISTRY_SECRET_NAME=$6
